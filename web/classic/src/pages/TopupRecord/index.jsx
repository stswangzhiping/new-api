    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div
        style={{
          fontSize: 22,
          fontWeight: 700,
          color: mainColor || 'var(--semi-color-text-0)',
          lineHeight: 1.2,
        }}
      >
        {mainVal}
...
const TopupHistoryPage = () => {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const map = {};
        users.forEach((u) => {
          map[u.id] = u.username || u.display_name || String(u.id);
        });
        setUsernameMap(map);
      }
    } catch {}
  ...
  const loadSystemGifts = useCallback(
    async (userId) => {
      try {
        const url =
          admin && userId
  ...
  const loadAllForStats = useCallback(async () => {
    ...
    try {
      ...
      if (rdRes.data?.success) {
        ...
        const items = Array.isArray(data) ? data : (data?.items ?? []);
        const merged = [...items, ...sysGifts].sort(
        ...
        if (admin) {
          ...
          if (usernameFilter) {
            items = items.filter((r) => {
              ...
              return (
                name.includes(usernameFilter) ||
                String(r.used_user_id).includes(usernameFilter)
              );
            });
          }
        }
  ...
  const columns = [
      ...
      : []),
    {
      ...
      render: (v, record) => {
        if (record._isSystemGift) {
          return (
            <Text style={{ color: 'var(--semi-color-text-3)', fontSize: 12 }}>
              —
            </Text>
          );
        }
    ...
    },
    {
      ...
      width: 100,
      render: (v, record) => (
        <SourceTag src={v} isSystemGift={record._isSystemGift} />
      ),
    },
    {
      title: t('有效期'),
